...
class SignupRemoteTests: XCTestCase {
    ...
    func testSuccessWhenStatusCodeIs2xx() {
        for _ in 0..<5 {
            test(withStatusCode: Int.random(in: 200..<300), email: "email@gmail.com", expectedResult: .success(nil))
        }
    }
    ...
    func testFailureWhenStatusCodeIs4xxOr5xx() {
        for _ in 0..<5 {
            ...
            test(withStatusCode: statusCode, email: "email@gmail.com", expectedResult: .failure(RemoteError(statusCode: statusCode)))
        }
    }
    ...
    private func test(withStatusCode statusCode: Int?, email: String, expectedResult: Result<Data?, RemoteError>) {
        urlSession.data = (nil,
